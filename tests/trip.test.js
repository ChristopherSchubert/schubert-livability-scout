...
import {
  ...
  transportDeepLinks, entriesByDay, tripDays, kindToV2,
  rowToTrip, tripToRow, rowToEntry, entryToRow,
  ENTRY_CATEGORIES, ENTRY_STATUSES,
} from "../lib/trip.js";
...
const trip = {
  ...
  entries: [
    { id: "pletna", day: "2026-05-19", category: "activity", status: "booked", title: "Pletna boat",
      time: { mode: "point", at: "14:00" }, cost: { amount: 36, currency: "EUR", payment: "onSite", cashOnly: true } },
    { id: "boat", day: "2026-05-18", category: "activity", status: "booked", title: "Vintage boat",
      time: { mode: "range", start: "10:00", end: "12:00" }, cost: { amount: 511, currency: "EUR", payment: "onSite", cashOnly: true } },
    { id: "para", day: "2026-05-20", category: "activity", status: "booked", title: "Paragliding",
      time: { mode: "bucket", bucket: "morning" }, cost: { amount: 380, currency: "EUR", payment: "onSite", cashOnly: true },
      booking: { confirmation: "FB-9921", cancelBy: "2026-05-13" } },
    { id: "franko", day: "2026-05-19", category: "meal", status: "reserved", title: "Hiša Franko",
      time: { mode: "point", at: "19:30" }, cost: { amount: 290, currency: "EUR", payment: "prepaid", cashOnly: true },
      booking: { cancelBy: "2026-05-12" } },
    { id: "lunch", day: "2026-05-18", category: "meal", status: "none", title: "Lunch",
      time: { mode: "bucket", bucket: "afternoon" }, cost: { amount: 40, currency: "EUR", payment: "onSite", cashOnly: false } },
    { id: "vintgar", day: "2026-05-19", category: "activity", status: "toBook", title: "Vintgar Gorge",
      time: { mode: "range", start: "11:00", end: "11:20" }, booking: { bookBy: "2026-05-01", leadTime: "weeks ahead" } },
  ],
};
...
describe("cashNeeded", () => {
  ...
  it("excludes prepaid even when cashOnly", () => {
    expect(cashNeeded({ entries: [{ cost: { amount: 9, currency: "EUR", payment: "prepaid", cashOnly: true } }] })).toEqual({});
  });
  it("ignores costs without a finite amount or currency", () => {
    expect(cashNeeded({ entries: [{ cost: { amount: NaN, currency: "EUR", payment: "onSite", cashOnly: true } }] })).toEqual({});
  });
...
describe("reservationLedger", () => {
  ...
  it("ranks lodging (stay) above non-lodging", () => {
    const led = reservationLedger({ entries: [
...
describe("transportDeepLinks", () => {
  it("builds a FlightAware link for flights", () => {
    expect(transportDeepLinks({ mode: "flight", carrier: "LH", number: "1462" }).flightAware)
      .toBe("https://flightaware.com/live/flight/LH1462");
  });
  it("builds a Google status query", () => {
    expect(transportDeepLinks({ mode: "flight", carrier: "LH", number: "1462" }).googleStatus)
  ...
  it("does not build FlightAware for non-flights", () => {
    expect(transportDeepLinks({ mode: "train", carrier: "OBB", number: "151" }).flightAware).toBe(null);
  });
});

describe("entriesByDay", () => {
  it("sorts timed entries by clock within a day", () => {
    expect(entriesByDay(trip)["2026-05-19"].map((e) => e.id)).toEqual(["vintgar", "pletna", "franko"]);
  });
...
describe("row <-> object mapping", () => {
  it("rowToTrip hydrates travelers/passes and tolerates nulls", () => {
    const t = rowToTrip({ id: "t1", user_id: "u1", name: "Slovenia", travelers: [{ name: "Chris" }], passes: [{ id: "p" }] });
  ...
  it("entry round-trips id/day/sort as columns, rest as payload", () => {
    const row = { id: "x", trip_id: "t", day: "2026-05-19", sort: 3, payload: { category: "meal", title: "Dinner" } };
    const e = rowToEntry(row);
    expect(e).toEqual({ id: "x", day: "2026-05-19", sort: 3, category: "meal", title: "Dinner" });
    const back = entryToRow("t", e);
    expect(back).toMatchObject({ trip_id: "t", day: "2026-05-19", sort: 3, payload: { category: "meal", title: "Dinner" } });
  });
});
...
describe("defensive edges", () => {
  ...
  it("rowToTrip coerces a Date start_date (pg pooler) to YYYY-MM-DD", () => {
    const t = rowToTrip({ id: "t", start_date: new Date(2026, 4, 18), end_date: new Date(2026, 4, 20) });
  ...
  it("rowToTrip falls back to empty collections on a sparse row", () => {
    ...
    expect(t).toMatchObject({ legs: [], options: {}, travelers: [], passes: [], entries: [], glance: {} });
  });
  ...
  it("tripToRow tolerates a bare trip object", () => {
    expect(tripToRow({})).toMatchObject({ user_id: null, name: "", legs: [], travelers: [], passes: [] });
  });
  it("tripDays falls back to glance.tz when a leg has none", () => {
    const days = tripDays({ startDate: "2026-05-18", endDate: "2026-05-18", glance: { tz: "America/New_York" }, legs: [] });

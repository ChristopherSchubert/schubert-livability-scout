...
// Trip-level resize — add or remove one calendar day at the START of the trip.
// Unlike a boundary shift (which keeps the trip span fixed), this moves the
// trip's startDate and grows/shrinks the FIRST leg only. delta = -1 starts the
// trip a day earlier (first leg gains a day); delta = +1 starts it a day later
// (first leg loses a day). Returns { startDate, legs } or null when the move
// would drop the first leg below one calendar day (the floor — a hard block).
export function resizeTripStart(startDate, legs, delta) {
  if (!startDate || !legs?.length) return null;
  const first = legs[0];
  const newArrive = addDays(first.arrive, delta);
  if (parseYmd(newArrive) > parseYmd(first.depart)) return null; // would invert
  const nextLegs = legs.map((l, i) => (i === 0 ? { ...l, arrive: newArrive } : l));
  return { startDate: addDays(startDate, delta), legs: nextLegs };
}

// Same, at the END of the trip — grows/shrinks the LAST leg. delta = +1 ends a
// day later (last leg gains a day); delta = -1 ends a day earlier (last leg
// loses a day). Returns { endDate, legs } or null at the one-day floor.
export function resizeTripEnd(endDate, legs, delta) {
  if (!endDate || !legs?.length) return null;
  const last = legs[legs.length - 1];
  const newDepart = addDays(last.depart, delta);
  if (parseYmd(newDepart) < parseYmd(last.arrive)) return null; // would invert
  const nextLegs = legs.map((l, i) => (i === legs.length - 1 ? { ...l, depart: newDepart } : l));
  return { endDate: addDays(endDate, delta), legs: nextLegs };
}

// ── Suggestion-tray ranking ─────────────────────────────────────────────────
// The Plan tab's city tray surfaces scouted Atlas cities NEAR the trip, not the
// whole atlas. "Near" = nearest-first to a set of anchor points (the trip's
// existing leg cities, or — at cold start with no legs yet — the trip name's
// geocoded center), keeping only places within `maxKm`. Pulled out of TripPlan
// so the Haversine + filter + sort can be unit-tested without a DOM.
const EARTH_KM = 6371;
...
// candidates/anchors: objects with numeric { lat, lon }. Returns the candidates
// within maxKm of the nearest anchor, nearest-first, capped at `limit`. Empty
// anchor set → [] (caller decides the no-anchor fallback; never invents one).
export function nearestCities(candidates, anchors, { maxKm = 400, limit = 12 } = {}) {
  const pts = (anchors || []).filter((a) => a && a.lat != null && a.lon != null);
  if (!pts.length) return [];
  return (candidates || [])
    .filter((c) => c && c.lat != null && c.lon != null)
    .map((c) => ({ c, d: Math.min(...pts.map((a) => haversineKm(a, c))) }))
    .filter((x) => x.d <= maxKm)
    .sort((a, b) => a.d - b.d)
    .slice(0, limit)
    .map((x) => x.c);

...
// placeableStops — the canonical "what goes on the map" split. Shared by
// TripMapInner and tripLimitations so they CANNOT disagree about the counts.
//
// Returns:
//   placed         — entries with real lat/lon (in day+time order)
//   placeableCount — total entries that could appear on the map (meal | activity)
//   unpinnedCount  — placeableCount minus those that actually have coords
//
// Rules (keep in sync with the TripMapInner render):
//   "placeable" = category "meal" or "activity"  (stays are reference, travel is
//   a connector — neither belongs as a numbered stop on the trip map)
//   "pinned"    = has e.place && e.place.lat != null && e.place.lon != null
export function placeableStops(trip) {
  const entries = trip.entries || [];
  const placeable = entries.filter((e) => e.category === "meal" || e.category === "activity");
  const placed = placeable.filter((e) => e.place && e.place.lat != null && e.place.lon != null);
  return {
    placed,
    placeableCount: placeable.length,
    unpinnedCount: placeable.length - placed.length,
  };
}
...
export function tripLimitations(trip, asOf) {
  const out = [];
  const entries = trip.entries || [];
  const { unpinnedCount } = placeableStops(trip);
  if (unpinnedCount) {
    out.push({
      severity: "warn",
      text: `${unpinnedCount} stop${unpinnedCount > 1 ? "s aren’t" : " isn’t"} pinned to a place — they won’t appear on the map or in travel-aware Solve.`,

...
export const EQUAL_WEIGHTS = { setting: 1, aliveness: 1, fabric: 1, realness: 1 };
...
export function metricSnapshot(city, m) {
  ...
  // Walking-core POI metrics (cafés / bars / restaurants / daily-needs) carry an
  // in_plateau/beyond breakdown from the measurer. Surface the real count of
  // places within the 15-min walk so the row leads with a number a human reads
  // directly — "6 cafés" — instead of the abstract weighted score (which keeps
  // driving the bar). The plateau split rides in a tooltip; see ChapterData.
  const meta = dp?.meta;
  const count =
    value != null && meta && meta.in_plateau != null
      ? { total: (meta.in_plateau || 0) + (meta.beyond || 0), inPlateau: meta.in_plateau || 0 }
      : null;
  return {
    key: m.key,
    label: m.label,
    ...
    barColor: metricBarColor(score, m.theme),
    direction: m.dir,
    asOf: dp?.asOf ?? null,
...
//
// A legacy metric (`supersededBy: "<key>"`, e.g. cafe_n → cafe_score) is folded
// INTO its superseding row as a compact `legacy` sub-stat rather than rendered
// as its own full row — the weighted score is the headline, the raw 700 m count
// rides along for sanity. The standalone legacy row is dropped only when the
// superseding metric is actually measured (mirrors axisRollup's rule), so a
// city not yet re-measured still shows the raw count on its own — no data lost.
export function axesSnapshot(city) {
  const rollup = axisRollup(city);
  return metricTaxonomy.map((group) => {
    const legacyOf = {}; // supersedingKey -> the legacy metric def it absorbs
    for (const m of group.metrics) if (m.supersededBy) legacyOf[m.supersededBy] = m;

    const metrics = [];
    for (const m of group.metrics) {
      // Superseded + its replacement is present → it folds in below, no own row.
      if (m.supersededBy && city.measuredMetrics?.[m.supersededBy]?.value != null) continue;
      const snap = metricSnapshot(city, m);
      const legacyDef = legacyOf[m.key];
      const ldp = legacyDef ? city.measuredMetrics?.[legacyDef.key] : null;
      // Only fold into a weighted row that actually has a value. If the weighted
      // metric is itself unmeasured, the legacy keeps its own standalone row
      // (it isn't skipped above) rather than hanging off an empty "—" row.
      if (ldp?.value != null && snap.value != null) {
        snap.legacy = {
          key: legacyDef.key,
          value: ldp.value,
          unit: legacyDef.unit,
          source: ldp.source || legacyDef.source,
          tagline: legacyDef.tagline,
        };
      }
      metrics.push(snap);
    }
    return { axis: group.axis, label: group.label, score: rollup[group.axis], metrics };
  });
}
...
export function buildCityDetailView(city, { slug, chipFrequencies = null } = {}) {
  return {
    ...city,
    ...
    chips: chipsFor(city, { frequencies: chipFrequencies }),
    axes: axesSnapshot(city),
    horizonFeatures: city.horizonFeatures ?? null,
  };
}

// The compact homebase (Allison Park) comparison the city-detail chapters render
// deltas against. Carries the climate window (When to go) AND a flat metric map
// keyed by metric key (By the numbers) so a row can look up "what does this read
// at home?" in O(1) without re-snapshotting on every render.
export function buildHomebaseView(homebase, { slug } = {}) {
  if (!homebase) return null;
  const axes = axesSnapshot(homebase);
  const metrics = {};
  for (const ax of axes) for (const m of ax.metrics) metrics[m.key] = m;
  return {
    name: homebase.name,
    slug: slug ?? homebase.slug ?? null,
    visitClimate: homebase.visitClimate,
    extremes: extremesFor(homebase),
    metrics, // { [metric_key]: metricSnapshot } — honest blanks: missing key ⇒ no reference shown
  };
}

...
export async function nearestWater(lat, lon) {
  ...
  for (const b of waterBodies(d.elements).values()) {
    ...
    if (r.dist != null && (best == null || r.dist < best.dist)) best = { ...r, body: b };
  }
  if (!best) return { dist: null, point: null, extentKm2: null };
  return { dist: best.dist, point: best.point, extentKm2: bodyExtentKm2(best.body) };
}
...
function waterBodies(elements) {
  ...
  const push = (key, name, kind, verts) => {
    let b = bodies.get(key);
    if (!b) { b = { name: null, kind, verts: [], bbox: [90, -90, 180, -180] }; bodies.set(key, b); }
    if (!b.name && name) b.name = name;
    for (const g of verts) {
      b.verts.push(g);
      const bb = b.bbox;
      if (g.lat < bb[0]) bb[0] = g.lat; if (g.lat > bb[1]) bb[1] = g.lat;
      if (g.lon < bb[2]) bb[2] = g.lon; if (g.lon > bb[3]) bb[3] = g.lon;
    }
  };
  for (const el of elements || []) {
    ...
    if (el.members) {
      const outers = el.members.filter((m) => m.geometry && m.role !== "inner");
      const area = outers.reduce((s, m) => s + ringAreaM2(m.geometry), 0);
      if (isSea || isRiver || area >= MIN_WATER_AREA_M2) for (const m of el.members) if (m.geometry) verts.push(...m.geometry);
    } else if (el.geometry) {
      if (isSea || isRiver || ringAreaM2(el.geometry) >= MIN_WATER_AREA_M2) verts.push(...el.geometry);
    } else if (el.lat != null && isSea) {
      verts.push({ lat: el.lat, lon: el.lon });
    }
    if (!verts.length) continue;
    if (isSea) push("sea", nat === "bay" || nat === "strait" ? name : null, "sea", verts);
    else if (isRiver) push(`river:${name || el.type + el.id}`, name, "river", verts);
    else push(`lake:${name || el.type + el.id}`, name, "lake", verts);
  }
...
// Spatial extent (km²) of a water body's bounding box — how far the water
// spreads near you. The ocean's coastline spans a huge box, a pond almost
// none, a lake/river between. Capped (the sea is effectively unbounded).
function bodyExtentKm2(b) {
  if (!b?.bbox) return null;
  const [minLat, maxLat, minLon, maxLon] = b.bbox;
  const latM = (maxLat - minLat) * 111320;
  const lonM = (maxLon - minLon) * 111320 * Math.cos(((minLat + maxLat) / 2) * Math.PI / 180);
  return Math.min(500, Math.round((Math.abs(latM * lonM) / 1e6) * 10) / 10);
}
...
export async function rankedWaterBodies(lat, lon, { max = 6 } = {}) {
  ...
  for (const b of waterBodies(d.elements).values()) {
    ...
    out.push({ name: b.name, kind: b.kind, dist: Math.round(best), point: { lat: +bp.lat.toFixed(5), lon: +bp.lon.toFixed(5) }, extentKm2: bodyExtentKm2(b) });
  }
...
export async function distanceToTarget(lat, lon, target) {
  ...
  return { ...minDistToVerts(lat, lon, match.verts), extentKm2: bodyExtentKm2(match) };
}

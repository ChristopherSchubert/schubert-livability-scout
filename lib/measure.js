...
// Nearest major water EDGE — nearest shoreline vertex, not a far-offshore bay
// centroid. Returns { dist, point } so callers can draw the line.
export async function nearestWater(lat, lon) {
  const d = await overpass(waterQuery(lat, lon, 15000));
  if (!d) return { dist: null, point: null };
  return minDistToVerts(lat, lon, majorWaterVerts(d.elements));
}
...
function waterBodies(elements) {
  ...
  const push = (key, name, kind, verts) => {
    ...
    if (!b) { b = { name: null, kind, verts: [] }; bodies.set(key, b); }
...
export async function rankedWaterBodies(lat, lon, { max = 6 } = {}) {
  ...
  for (const b of waterBodies(d.elements).values()) {
    ...
    out.push({ name: b.name, kind: b.kind, dist: Math.round(best), point: { lat: +bp.lat.toFixed(5), lon: +bp.lon.toFixed(5) } });
  }
...
export async function distanceToTarget(lat, lon, target) {
  ...
  if (!match || bestToPoint > 2000) return { dist: null, point: null }; // body not found nearby
  return minDistToVerts(lat, lon, match.verts);
}
...
async function elevations(points) {
  ...
  for (let i = 0; i < points.length; i += 100) {
    ...
    const url = `https://api.open-meteo.com/v1/elevation?latitude=${c.map((p) => p[0].toFixed(5)).join(",")}&longitude=${c.map((p) => p[1].toFixed(5)).join(",")}`;
    let d = null;
    for (let t = 0; t < 4 && !d?.elevation; t++) {
      try { const r = await fetch(url, { headers: { "User-Agent": UA } }); d = await r.json(); if (!d.elevation) await sleep(1500); }
      catch { await sleep(1500); }
    }
    out.push(...(d?.elevation || c.map(() => null)));
    await sleep(300);
  }
  return out;
}

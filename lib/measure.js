...
export async function measureWaterHorizon(lat, lon, { asOf } = {}) {
  ...
  const sectorMin = new Array(16).fill(Infinity);
  for (const b of bodies) for (const g of b.verts) {
    const dist = haversine(lat, lon, g.lat, g.lon);
    if (dist > 15000) continue;
    const sec = Math.round(bearingDeg(lat, lon, g.lat, g.lon) / 22.5) % 16;
    if (dist < sectorMin[sec]) sectorMin[sec] = dist;
  }
  const open = sectorMin.filter((dm) => dm <= 12000).length;

...
//
// maxDriftM caps how far a candidate may sit from the anchor — this is the
// "stay inside the neighborhood I picked" guardrail. Without it, a wide net
// around (say) South Side Slopes finds downtown Pittsburgh, which technically
// has more cafés but isn't what the user asked about. Pass null to disable
// (e.g. when the anchor is a rough city-level geocode).
export async function findVisitCenters(lat, lon, { max = 4, maxDriftM = 1200 } = {}) {
  ...
  while (pool.length >= 5 && peaks.length < max) {
    ...
    // genuinely different peak — but only KEEP it if it sits inside the drift
    // budget. Otherwise we'd just re-surface the same too-far peak.
    pool = pool.filter((p) => haversine(cen.lat, cen.lon, p.lat, p.lon) > VC_SEP);
    if (maxDriftM != null && drift > maxDriftM) continue;

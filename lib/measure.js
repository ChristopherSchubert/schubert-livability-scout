...
function minDistToVerts(lat, lon, verts) {
  let best = null;
  for (const g of verts) { const d = haversine(lat, lon, g.lat, g.lon); if (best == null || d < best) best = d; }
  return best == null ? null : Math.round(best);
}

// Distance (m) to the nearest major water EDGE — nearest shoreline vertex, not
// a far-offshore bay centroid.
...
export async function nearestWaterMulti(lat, lon, points) {
  ...
  return points.map((p) => minDistToVerts(p.lat, p.lon, verts));
}
...
export async function distanceToTarget(lat, lon, target) {
  ...
  if (!match || bestToPoint > 2000) return null; // target body not present nearby

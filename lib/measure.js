...
// Fetch a polygon for a named stay zone (e.g. "South Side Slopes, Pittsburgh,
// PA") from Nominatim. Returns a GeoJSON Polygon / MultiPolygon, or null if
// Nominatim has no polygon for that string. Used by the scout endpoint to
// constrain candidate cores to inside the neighborhood the user picked.
export async function fetchStayZoneBoundary(stayZone, cityName) {
  if (!stayZone) return null;
  const cityPart = (cityName || "").split(",")[0].trim();
  const statePart = ((cityName || "").split(",")[1] || "").trim();
  // Try the most specific query first, then back off. Nominatim's neighborhood
  // coverage is patchy — sometimes the bare "stay zone, state" works when the
  // city-prefixed query doesn't (or vice versa).
  const tries = [
    `${stayZone}, ${cityPart}${statePart ? ", " + statePart : ""}`,
    `${stayZone}, ${cityPart}`,
    `${stayZone}${statePart ? ", " + statePart : ""}`,
  ];
  for (const q of tries) {
    try {
      const params = new URLSearchParams({ q, format: "json", polygon_geojson: "1", limit: "1" });
      const d = await (await fetch(`https://nominatim.openstreetmap.org/search?${params}`, { headers: { "User-Agent": UA } })).json();
      const hit = d?.[0];
...
const MAJOR_RIVERS = new Set([
  ...
  // Substantial regional rivers (added on a per-case basis when a city in
  // the dataset sits on one and OSM under-maps it):
  "French Broad River", // Asheville — navigable, ~150m wide through downtown
]);

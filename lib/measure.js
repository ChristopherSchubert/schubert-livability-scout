...
// Overpass sometimes returns 200 OK with a `remark` field signalling the query
// timed out, exceeded memory, or was otherwise killed — the body is then partial
// or empty. Computing metrics from it writes "zero of everything" as if measured
// (the 2026-06-04 Bled/Ljubljana/Piran incident). Every Overpass-backed measurer
// must treat such a response as a FAILURE, never as data. (CLAUDE.md Corollary 2.)
export function overpassRemarkFailed(d) {
  return !!(d && d.remark && /timeout|timed out|time limit|killed|memory|empty|runtime|out of|exceeded/i.test(d.remark));
}
...
export async function osmMetrics(lat, lon, { landFraction = 1.0 } = {}) {
  const q = `[out:json][timeout:90];
    ...
    out geom;`;
  const d = await overpass(q);
  if (!d || overpassRemarkFailed(d)) return {}; // failed/degraded → never write zeros
  const out = {};
...
export async function measureCensus(lat, lon, apiKey, { asOf, year = 2023 } = {}) {
  ...
  //   B08301_010E — workers commuting by public transportation (incl. bus, subway, etc.)
  //   B08301_019E — workers commuting on foot (walked)
  ...
  //   B25034_011E — units built 1939 or earlier (pre-1940). NOTE: _010E is
  //     "Built 1940 to 1949" in the current B25034 vintage — using it was the
  //     2026-06-07 off-by-one that read the 1940s decade as "pre-1940".
  //   B25077_001E — median home value (owner-occupied)
  const vars = "B01003_001E,B08301_001E,B08301_010E,B08301_019E,B19013_001E,B25001_001E,B25003_001E,B25003_002E,B25004_006E,B25034_001E,B25034_011E,B25077_001E";
  ...
  const walkCommute = num(m.B08301_019E);
  const transitCommute = num(m.B08301_010E);
  ...
  const pre1940 = num(m.B25034_011E);

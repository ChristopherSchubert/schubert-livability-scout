  ...
  try {
    ...
    }
    {
      ...
      const mo = Array.from({ length: 12 }, () => ({
        hi: 0, lo: 0, mean: 0, dew: 0, hidx: 0,
        n: 0, nMean: 0, nDew: 0, nHidx: 0, wet: 0, day: 0, nDay: 0,
        precipMm: 0,
      }));
      // Per-day heat index in °F using the NWS Rothfusz regression, with
      // Magnus to derive RH from dewpoint. Below 80°F or 40% RH the formula
      // collapses to the air temperature — that matches how NWS treats it.
      // We average daily-max heat-index per month so the chart's "Feels"
      // row pairs cleanly against the daily-max "High" row.
      const heatIndexF = (tcMax, tdC) => {
        if (tcMax == null || tdC == null) return null;
        const tF = tcMax * 9 / 5 + 32;
        if (tF < 80) return tF;
        // Magnus formula → vapor pressures → RH%.
      ...
      for (const date of Object.keys(param.T2M_MAX).sort()) {
        ...
        const hidx = heatIndexF(hi, dew);
        if (hidx != null) { mo[m].hidx += hidx; mo[m].nHidx += 1; }
      ...
      visitClimate = mo.map((x) => x.n ? {
        ...
        heatIndex: x.nHidx ? Math.round((x.hidx / x.nHidx) * 10) / 10 : null,

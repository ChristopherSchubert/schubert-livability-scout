...
export async function measureWaterHorizon(lat, lon, { asOf } = {}) {
  ...
  const d = await overpass(waterQuery(lat, lon, 15000));
  if (!d) return { metrics: {} };
  // Qualifying open water: the sea, a sizable lake (≥5 km²), or a real named
  // river that runs through the town (its channel comes within ~2.5 km of the
  // center). A river out in the hills doesn't count; the Columbia at Hood River
  // or the Savannah River do.
  const nearestVert = (b) => b.verts.reduce((m, g) => Math.min(m, haversine(lat, lon, g.lat, g.lon)), Infinity);
  // Qualifying open water: the sea; a sizable lake (≥5 km²); or a genuinely wide
  // river through the town (≥1 km² of water surface AND its channel within
  // 2.5km — so the wide Columbia counts but a narrow creek-river like the
  // Rivanna doesn't). The sea reads from far (12km); inland water only counts
  // when it's near enough to actually see (5km) — a river 10km off isn't on
  // your horizon.
  const bodies = [...waterBodies(d.elements).values()].filter(
    (b) => b.isSea || b.areaM2 >= 5e6 || (b.kind === "river" && b.areaM2 >= 1e6 && nearestVert(b) <= 2500),
  );

...
// Above this bbox area (km²) a polygon is "city-scale" — too coarse to be a
// useful neighborhood boundary. The whole point of the constraint is to keep
// candidate cores inside the lived-in zone, not "anywhere in Santa Cruz."
// Most named US neighborhoods fit under ~6 km²; 10 leaves room for the
// generous ones while still rejecting the city polygons. When we hit this
// case we'd rather draw a 700 m circle around a meaningful point than store
// a polygon that effectively says "no constraint."
const MAX_BOUNDARY_AREA_KM2 = 10;

// Approximate area of a GeoJSON polygon by its bbox. Cheap; close enough for
// a "is this thing city-scale?" check at neighborhood latitudes.
function geojsonBboxAreaKm2(geo) {
  ...
  if (!isFinite(minLat)) return Infinity;
  const midLat = (minLat + maxLat) / 2;
  const hKm = (maxLat - minLat) * 111.32;
...
export async function fetchStayZoneBoundary(stayZone, cityName, anchor = null) {
  ...
  // Tokens from the stay_zone, split on "/", with light normalization:
  //   - strip any parenthetical aside ("Hampton Twp (owner suburb)" → "Hampton Twp")
  //   - expand common admin abbreviations ("Twp" → "Township") so OSM
  //     boundary relations match
  //   - drop "downtown" since it's generic and never resolves to a polygon
  const expandAbbrevs = (s) => s
    .replace(/\bTwp\b\.?/gi, "Township")
    .replace(/\bSq\b\.?/gi, "Square")
    .replace(/\bSt\b\.?/gi, "Street");
  // Trim trailing descriptors that aren't part of an OSM place name:
  // "Beach Hill edge" → "Beach Hill", "downtown core" → "downtown".
  const stripSuffix = (s) => s.replace(/\s+(edge|area|neighbou?rhood|district|core|side)$/i, "").trim();
  // Split on "/" and "+" so "Fairhaven + downtown" yields ["Fairhaven","downtown"].
  const rawTokens = (stayZone || "").split(/\s*[/+]\s*/);
  const zoneTokens = rawTokens
    .map((t) => stripSuffix(expandAbbrevs(t.replace(/\s*\([^)]*\)\s*/g, " ").trim())))
    .filter(Boolean);
  // Keep "downtown" as a token — "Downtown Hood River" / "Downtown Frederick"
  // are real OSM features even though bare "downtown" never resolves alone.
  const tries = [
    ...
    // No bare-city fallback: the whole-city polygon is invariably too coarse
    // to be a useful neighborhood boundary (downtown is *inside* the city
    // polygon, so constraining to it doesn't actually constrain anything).
    // We'd rather fall through to the Point→circle path below, or to the
    // saved-anchor circle, than save "all of Santa Cruz" as the stay zone.
  ].filter(Boolean);
  ...
  for (let i = 0; i < uniq.length; i++) {
    ...
    if (!hit) continue;
    // Reject city-scale polygons — see MAX_BOUNDARY_AREA_KM2.
    if (geojsonBboxAreaKm2(hit) > MAX_BOUNDARY_AREA_KM2) continue;
    return hit;
  }
  // Polygon search via stay_zone tokens exhausted. Before falling back to
  // synthetic circles, ASK OSM what neighborhood is at the saved center —
  // reverse geocode pulls the address hierarchy (neighbourhood / suburb /
  // quarter / city_district) and any of those is a "clear best neighborhood"
  // for that point. We then look up each in turn for its polygon and return
  // the first one that's actually neighborhood-scale.
  if (anchor && Number.isFinite(anchor.lat) && Number.isFinite(anchor.lon)) {
    ...
    for (const n of names) {
      ...
      if (hit && geojsonBboxAreaKm2(hit) <= MAX_BOUNDARY_AREA_KM2) return hit;
    }
  }
  // Polygon search exhausted — fall back to the first Point hit and
  // synthesize a circle around it. Some real neighborhoods (Rochester's South
  // Wedge, smaller CDPs) live in OSM only as a single tagged node; a circle
  // around the canonical point is still a meaningfully tighter constraint
  // than the city-wide drift cap, and at least makes the boundary visible.
  for (let i = 0; i < uniq.length; i++) {
    if (i > 0) await sleep(1100);
    const pt = await nominatimSearchPoint(uniq[i]);
    if (pt) return circlePolygon(pt.lat, pt.lon, 700);
  }
  // Last resort — if the caller gave us the saved center, draw a circle there
  // so the boundary still exists and constrains scouting. Without this a
  // city like "Santa Cruz, CA [Downtown / Beach Hill edge]" — where neither
  // token resolves — would have no boundary at all.
  if (anchor && Number.isFinite(anchor.lat) && Number.isFinite(anchor.lon)) {
    return circlePolygon(anchor.lat, anchor.lon, 800);
  }
  return null;
}

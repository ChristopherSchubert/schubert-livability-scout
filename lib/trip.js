...
export function tripDays(trip) {
  ...
  for (let t = start.getTime(); t <= end.getTime(); t += DAY_MS) {
    ...
    out.push({ date, cityId: leg?.cityId ?? null, legName: leg?.name ?? null, tz: leg?.tz ?? trip.glance?.tz ?? null });
  }
  return out;
}
...
export function reservationLedger(trip) {
  ...
  return rows.sort(
    (a, b) => isLodging(a) - isLodging(b) || deadline(a).localeCompare(deadline(b))
  );
}

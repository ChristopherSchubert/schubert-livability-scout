...
// Partial trip-FRAME patch (camelCase) → snake columns, for updateTrip (#11).
// Unknown keys are dropped, not thrown (the mapPatch discipline); `entries` is
// deliberately unmappable (entries live in trip_entries). Pure + tested (#42).
const TRIP_PATCH_COLS = {
  name: "name",
  theme: "theme",
...
export function tripPatchToRow(patch) {
  const out = {};
  for (const [k, v] of Object.entries(patch || {})) {
    if (!TRIP_PATCH_COLS[k]) continue;
    out[TRIP_PATCH_COLS[k]] = v === undefined ? null : v;
  }
  return out;
}

...
export const metricTaxonomy = [
  {
    ...
    metrics: [
      { key: "skyline_deg",      label: "Mountain backdrop",      tagline: "Steepest visible horizon rise",         unit: "°",   dir: +1, theme: "stone", source: "Open-Meteo elevation (line-of-sight skyline)", sourceUrl: "https://open-meteo.com/en/docs/elevation-api" },
      { key: "mtn_horizon_pct",  label: "Mountains on the horizon", tagline: "How much of the horizon is mountains", unit: "%",   dir: +1, theme: "stone", source: "Open-Meteo elevation + OSM peaks", sourceUrl: "https://open-meteo.com/en/docs/elevation-api" },
      { key: "water_dist_m",     label: "Distance to water",      tagline: "To nearest sea, river, or lake edge",   unit: "m",   dir: -1, theme: "water", source: "OpenStreetMap (Overpass)",    sourceUrl: "https://overpass-api.de" },
      { key: "water_extent_km2", label: "Open water",             tagline: "Ocean, bay, lake — or just a pond",     unit: "km²", dir: +1, theme: "water", source: "OpenStreetMap (Overpass)",    sourceUrl: "https://overpass-api.de" },
    ],
  },
  {
    ...
    metrics: [
      ...
      { key: "cafe_score", label: "Cafés", tagline: "Cafés within a 15-min walk", unit: "score", dir: +1, source: "Google Places (New) via local cache", sourceUrl: "https://developers.google.com/maps/documentation/places/web-service" },
      { key: "bar_score",  label: "Bars",  tagline: "Bars & pubs within a 15-min walk", unit: "score", dir: +1, source: "Google Places (New) via local cache", sourceUrl: "https://developers.google.com/maps/documentation/places/web-service" },
      { key: "rest_score", label: "Restaurants", tagline: "Restaurants within a 15-min walk", unit: "score", dir: +1, source: "Google Places (New) via local cache", sourceUrl: "https://developers.google.com/maps/documentation/places/web-service" },
  ...
  },
  {
    ...
    metrics: [
      { key: "intersection_den", label: "Intersection density", tagline: "Where 3+ streets meet, per km²",  unit: "/km²", dir: +1, source: "OpenStreetMap (osmnx)", sourceUrl: "https://www.openstreetmap.org" },
      ...
      { key: "bldg_coverage",    label: "Building coverage",    tagline: "How densely the core is built up", unit: "frac", dir: +1, source: "OpenStreetMap (osmnx)", sourceUrl: "https://www.openstreetmap.org" },
  ...
  },
  {
    ...
    metrics: [
      { key: "daily_needs_score", label: "Daily-needs shops", tagline: "Groceries, pharmacy & bakery within a 15-min walk", unit: "score", dir: +1, source: "Google Places (New) via local cache", sourceUrl: "https://developers.google.com/maps/documentation/places/web-service" },
      ...
      { key: "core_density",     label: "Core density",         tagline: "People per square mile in the core",      unit: "/sqmi", dir: +1, source: "US Census ACS (tract)",    sourceUrl: "https://data.census.gov" },
      ...
      { key: "median_price_usd", label: "Median home value",    tagline: "Typical home value in the core",          unit: "$",     dir: -1, source: "US Census ACS (B25077)",    sourceUrl: "https://data.census.gov" },
  ...
  {
    // #105: dropped from vacation scoring + renamed "Year-round" → "Off-season".
    // Off-season hardiness isn't what makes a place a good *visit* — a Florida
    // pin shouldn't outrank a New England pin on Fit because Floridians don't
    // see snow. The data still renders in Chapter IV (per-metric rows) and the
    // year-shape curve in Chapter V, but the axis no longer rolls into Fit or
    // shows as a Compare-table column. `excludeFromScoring` is consumed by
    // `calibrateAxes` (below) which drives scoring + Compare's axis columns.
    // Internal `axis` key stays "january" so the felt_surveys.january column +
    // baseline_ratings.january column round-trip unchanged.
    axis: "january", label: "Off-season", excludeFromScoring: true,
    metrics: [
      { key: "pleasant_days",     label: "Pleasant days / yr",  tagline: "Mean 55–80°F and dry",                    unit: "days",  dir: +1, theme: "sun", source: "NASA POWER (MERRA-2) daily archive (2019–2023)", sourceUrl: "https://power.larc.nasa.gov/docs/services/api/temporal/daily/" },
      ...
      { key: "clear_days",        label: "Sunny days / yr",     tagline: "Sun out most of the day",                 unit: "days",  dir: +1, theme: "sun", source: "NASA POWER (MERRA-2) daily archive (2019–2023)", sourceUrl: "https://power.larc.nasa.gov/docs/services/api/temporal/daily/" },
    ],
  },
];
...
// ── METRIC BAR COLOR — encodes GOODNESS, not polarity ───────────────────
// The detail-page bars used to be colored by `dir` (teal for higher-is-better,
// red for lower-is-better). That painted a city's BEST "lower is better" stats
// (right on the water, fine-grained blocks) in the warning color — color
// disagreed with bar length. Color now tracks the 0–10 score: weak → muted
// clay, strong → deep teal. Because the score already bakes in direction (the
// band for `water_dist_m` runs 1200 m → 150 m), one ramp serves both "low is
// bad" and "low is good" metrics correctly.
//
// A handful of metrics carry a `theme` (set in the taxonomy) that shifts the
// GOOD (high-score) end toward a natural hue — only where it reinforces
// goodness rather than fighting it. Deliberately NOT themed: the temperature
// day-counts (hot/cold days). A literal hot=red / cold=blue gradient would
// paint the bar loudest on its WORST end, reintroducing the good/bad
// confusion the score-driven coloring exists to remove. They stay neutral.
//
// Each ramp is [score, [r,g,b]] stops; the bar color is the piecewise-linear
// interpolation at the metric's score. Stays inside the paper/teal palette —
// not a harsh red→green verdict scale (the project is anti-scorecard).
const BAR_RAMPS = {
  // weak → middling → strong → excellent
  neutral: [[0, [0xc0, 0x84, 0x6f]], [4, [0xcb, 0xa1, 0x64]], [6.5, [0x5d, 0x90, 0x77]], [10, [0x0d, 0x4c, 0x44]]],
  water:   [[0, [0xc0, 0x84, 0x6f]], [4, [0xc4, 0xa2, 0x77]], [6.5, [0x4c, 0x8a, 0x93]], [10, [0x15, 0x59, 0x6a]]],
  stone:   [[0, [0xc0, 0x84, 0x6f]], [4, [0xb6, 0x9a, 0x82]], [6.5, [0x5b, 0x83, 0x85]], [10, [0x2f, 0x5d, 0x63]]],
  // sun gets a cool→warm ramp (overcast grey → gold): "more sun = brighter &
  // warmer = good", so contrast reads as goodness without any ambiguity.
  sun:     [[0, [0x8f, 0x9a, 0xa0]], [4.5, [0xd3, 0xb2, 0x66]], [10, [0xe0, 0xa5, 0x1c]]],
};
...
// Hex bar color for a 0–10 score under an optional theme. Null score → null
// (the bar renders empty — "not yet measured", never a fabricated color).
export function metricBarColor(score, theme) {
  if (score == null) return null;
  const ramp = BAR_RAMPS[theme] || BAR_RAMPS.neutral;
  const s = Math.max(0, Math.min(10, score));
  for (let i = 0; i < ramp.length - 1; i++) {
    const [lo, loRgb] = ramp[i];
    const [hi, hiRgb] = ramp[i + 1];
    if (s <= hi) {
      const t = hi === lo ? 0 : (s - lo) / (hi - lo);
      return "#" + lerpByte(loRgb[0], hiRgb[0], t) + lerpByte(loRgb[1], hiRgb[1], t) + lerpByte(loRgb[2], hiRgb[2], t);
    }
  }
  const last = ramp[ramp.length - 1][1];
  return "#" + lerpByte(last[0], last[0], 0) + lerpByte(last[1], last[1], 0) + lerpByte(last[2], last[2], 0);
}

// The measured axes that contribute to the vacation Fit score (and show as
// columns on Compare). Filters out taxonomy groups flagged `excludeFromScoring`
// (#105: off-season hardiness, which isn't a "good visit" signal — see the
// comment on the "january" axis above). Today that leaves 4 of 5 axes.
export const calibrateAxes = metricTaxonomy
  .filter((g) => !g.excludeFromScoring)
  .map((g) => [g.axis, g.label]);

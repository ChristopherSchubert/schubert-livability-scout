// Felt-score survey (Track 2) — the subjective axes + the 0–10 Slovenia score,
// the regression target the measured metrics aim to predict. Extracted from the
// lib/planner-data.js godfile (issue #47); a self-contained leaf (no deps on the
// rest of the model). planner-data.js re-exports these so existing imports are
// unchanged. See CLAUDE.md "the three data tracks".
...
export const surveyAxes = [
  {
    ...
    label: "Setting",
    prompt:
      "Does where you are press in on you — water, terrain, prospect, a view that reorients you?",
    anchors: [
  ...
  },
  {
    ...
    label: "Aliveness",
    prompt:
      "On an ordinary weekday — not events, not weekends — are people out and lingering with no errand?",
    anchors: [
  ...
  },
  {
    ...
    label: "Fabric",
    prompt:
      "Does the built fabric close around you — enclosure, human scale, a core that holds together?",
    anchors: [
  ...
  },
  {
    ...
    label: "January test",
    prompt:
      "Would there be outdoor public life on a gray Tuesday in February — or does it shutter?",
    anchors: [
...
export function surveyComplete(survey) {
  ...
  return (
    surveyAxes.every((axis) => Number.isFinite(survey[axis.key])) &&

...
export function deriveMarkers(poi) {
  ...
  // A vegan/vegetarian primary_type IS a cited veg signal — Google categorized
  // the place that way — and it's far more reliable than the sparse
  // servesVegetarianFood attribute (which is unset even for vegan restaurants).
  if (/\b(vegan|vegetarian)_restaurant\b/.test(poi.primary_type || "")) {
    markers.push({ type: "veg", source: "Google Places · type" });
  }
  ...
  if (a && typeof a === "object") {
    ...
    if (a.servesVegetarianFood === true && !markers.some((m) => m.type === "veg")) markers.push({ type: "veg", source });

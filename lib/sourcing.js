...
/** @type {Array<[RegExp, { kind: string, category: string }]>} */
const TYPE_MAP = [
  [
    /restaurant|cafe|coffee|bar$|bar_and_grill|bakery|dessert|ice_cream|food|meal_|bistro|pub|brunch|brewery|wine|tea_house/,
    { kind: "meal", category: "Eat & drink" },
  ],
  [
    /hotel|lodging|bed_and_breakfast|guest_house|resort|hostel|motel|cottage|inn/,
    { kind: "checkin", category: "Stay" },
  ],
  [
    /castle|museum|gallery|tourist_attraction|historical|landmark|monument|amphitheatre|cultural/,
    { kind: "flexible", category: "See" },
  ],
  [
    /park|hiking|trail|nature|garden|beach|lake|scenic|viewpoint|zoo|aquarium|wildlife/,
    { kind: "flexible", category: "Outdoors" },
  ],
  [
    /adventure|sports|zipline|climbing|ski|kayak|rafting|amusement|water_park|spa|wellness/,
    { kind: "booked", category: "Do" },
  ],
...
export function confidenceScore(poi) {
  const r = poi.rating || 0,
    n = poi.user_rating_count || 0;
...
export function deriveMarkers(poi) {
  ...
  if (a && typeof a === "object") {
    const when = poi.attributes_fetched_at
      ? new Date(poi.attributes_fetched_at).toISOString().slice(0, 10)
      : null;
    ...
    if (
      a.accessibilityOptions?.wheelchairAccessibleEntrance === true ||
...
/** @param {any[]} rawPois @param {{ origin?: { lat: number, lon: number } }} [opts] */

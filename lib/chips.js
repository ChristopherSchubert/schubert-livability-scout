...
const CHIPS = [
  ...
  {
    id: "historic", group: "urban-form", label: "Historic", specificity: 18,
  ...
  {
    id: "cafe-dense", group: "public-life", label: "Café culture", specificity: 15,
  ...
  {
    id: "trails-out-the-door", group: "outdoors", label: "Hiking nearby", specificity: 25,
...
// Every firing chip for this city — no group dedup, no cap. This is the
// data-layer answer: a city qualifies for every chip whose threshold it
// meets, and that's what filters (Calibrate) check against. Peninsula AND
// Coastal both fire for a peninsular city, so a "Coastal" filter matches.
export function allChipsFor(city) {
  if (!city) return [];
  return CHIPS.filter((c) => c.test(city)).map((c) => c.label);
}

// Frequency map over a corpus — label → number of cities firing it. Used
// to rank the display strip rarest-first ("Mild winter" beats "Walkable"
// because it's a more distinguishing fact).
export function chipFrequencies(cities) {
  const f = new Map();
  if (!cities) return f;
  for (const c of cities) for (const lbl of allChipsFor(c)) f.set(lbl, (f.get(lbl) ?? 0) + 1);
  return f;
}
...
// Climate groups are editorially important — Real winter / Mild winter /
// Humid summer carry signal that the strip should not silently drop just
// because four earlier-group chips happened to fire. If a climate chip
// would be capped out, evict the lowest-specificity non-climate winner to
// reserve its slot. Only consulted in the no-frequencies fallback; rarity
// ranking gives climate chips priority naturally.
const RESERVED_GROUPS = new Set(["climate-winter", "climate-summer"]);

// Display strip — at most `max` chips, ranked by rarity when `frequencies`
// is provided, else by the group-order + climate-eviction fallback. Within
// a group only one chip survives (no Peninsula + Coastal in the same strip).
export function chipsFor(city, { max = 4, frequencies = null } = {}) {
  if (!city) return [];

  if (frequencies) {
    // Within each group, keep the rarest firing chip (specificity tiebreak).
    const byGroup = new Map();
    for (const chip of CHIPS) {
      if (!chip.test(city)) continue;
      const freq = frequencies.get(chip.label) ?? 0;
      const prev = byGroup.get(chip.group);
      if (!prev ||
          freq < prev.freq ||
          (freq === prev.freq && chip.specificity > prev.chip.specificity)) {
        byGroup.set(chip.group, { chip, freq });
      }
    }
    return [...byGroup.values()]
      .sort((a, b) => a.freq - b.freq ||
        GROUP_ORDER.indexOf(a.chip.group) - GROUP_ORDER.indexOf(b.chip.group))
      .slice(0, max)
      .map(({ chip }) => chip.label);
  }

  // No corpus → fall back to group-order + climate-slot reservation.
  const winners = pickByGroup(city);
  const ordered = GROUP_ORDER.map((g) => winners.get(g)).filter(Boolean);
  if (ordered.length <= max) return ordered.map((c) => c.label);

  const kept = ordered.slice(0, max);
  const dropped = ordered.slice(max);
  for (const chip of dropped) {
    if (!RESERVED_GROUPS.has(chip.group)) continue;
    let evictIdx = -1;
    let evictSpec = Infinity;
    for (let i = 0; i < kept.length; i++) {
      if (RESERVED_GROUPS.has(kept[i].group)) continue;
      if (kept[i].specificity < evictSpec) {
        evictSpec = kept[i].specificity;
        evictIdx = i;
      }
    }
    if (evictIdx >= 0) kept[evictIdx] = chip;
  }
  kept.sort((a, b) => GROUP_ORDER.indexOf(a.group) - GROUP_ORDER.indexOf(b.group));
  return kept.map((c) => c.label);
}

...
//
// #92 (epic #84): the pipeline now writes to the shared schubert-family project,
// `travel` schema — NOT the legacy schubert-travel. TWO SECRET STORES, easy to
// confuse: this uses `supabase-family-db-password` (the family pooler), NOT
// travel's live `supabase-db-password` slot. `search_path=travel` so the
// unqualified table names below (`cities`, …) resolve to `travel.*`. Measurement
// is local-only (prod never measures), so new measurements land in family.travel
// and the live app sees them once the app env flips at cutover (#91).
function dbPassword() {
  return execFileSync(
    "security",
    ["find-generic-password", "-a", "livability-scout", "-s", "supabase-family-db-password", "-w"],
    { encoding: "utf8" }
  ).trim();
}

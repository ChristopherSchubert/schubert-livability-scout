...
async function nominatimFallback(text, parsed, _bbox, cityCenter) {
  // NO_NOMINATIM=1 → stay fully local (Overpass only). The public Nominatim
  // fallback is slow (throttled) and was preventing the resolve pass from ever
  // finishing; skipping it leaves blocks Overpass can't place as "unresolved"
  // (placeholder card) rather than blocking the whole run on a public mirror.
  if (process.env.NO_NOMINATIM) return unresolved({ skipped: "nominatim disabled (NO_NOMINATIM)" });

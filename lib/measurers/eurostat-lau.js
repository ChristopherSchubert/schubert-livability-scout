...
export default {
  ...
  writes: {
    ...
    columns: ["eu_lau", "population_total", "population_source"],
  },
  ...
  async run({ lat, lon, asOf }) {
    ...
    // Surface the LAU population as the city-wide population_total too — the
    // EU analogue of the census measurer's Place population, and the per-capita
    // denominator the crowd_season pipeline needs. LAU municipality ≈ Census
    // Place (the town's administrative unit); the differing ruler is recorded
    // in population_source, consistent with the US Place/cousub/ZCTA tiers.
    if (lau.pop_2021 != null) {
      columns.population_total = lau.pop_2021;
      columns.population_source = `eurostat_gisco_lau_${LAU_VINTAGE}:${lau.gisco_id}:${lau.lau_name}`;
    }

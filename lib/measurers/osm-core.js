...
import { osmMetrics, measureBuildingCoverage } from "../measure.js";
...
export default {
  ...
  // Two Overpass calls per city. Local container shrugs this off; public mirror
  // throttles at ~1 city / 5 s, which is why this measurer is intended to be
  // run with OVERPASS_URL pointing at the local container.
  throttleMs: 800,
  async run({ lat, lon, asOf }) {
    const [raw, bldg] = await Promise.all([
      osmMetrics(lat, lon),
      measureBuildingCoverage(lat, lon),
    ]);
    ...
    const note = `cafe ${raw.cafe_n ?? "?"} | rest ${raw.rest_n ?? "?"} | bar ${raw.bar_n ?? "?"} | needs ${raw.daily_needs_n ?? "?"} | bldg ${bldg.bldg_coverage?.value ?? "?"}`;

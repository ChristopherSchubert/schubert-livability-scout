...
export async function runForCity(client, city, measurers, { asOf, force = false, dryRun = false }) {
  ...
  for (const m of order) {
    ...
    const ctx = {
      ...
      // The shared Postgres client. Most measurers don't need it — they hit
      // external APIs. The walking-core measurer reads from the local `pois`
      // cache (populated by scripts/.fetch-pois.mjs) so it needs DB access.
      db: client,
      env: process.env,
    };

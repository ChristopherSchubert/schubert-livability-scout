...
// Overpass request — the public mirror handles it in ~30s for most cities.
//
// TODO(local-overpass): respects OVERPASS_URL via lib/measure.js#overpass.
// When the local container is up, switch to it for batch backfills —
// the public mirror rate-limits hard above ~1 city per ~5s; local is unbounded.
//
...
import { overpass, haversine, ringAreaM2 } from "../measure.js";
...
export default {
  ...
  // Single heavy Overpass call — pause generously so the public mirror doesn't
  // 429. Drop this when OVERPASS_URL points at the local container.
  throttleMs: 4000,
  async run({ lat, lon, asOf }) {
    const d = await overpass(osmContextQuery(lat, lon));
    if (!d) return { notes: "Overpass returned nothing (rate-limited or down)" };

// lib/measurers/census.js — US Census ACS tract-level metrics.
//
// Wraps measureCensus from lib/measure.js. Needs CENSUS_API_KEY in env.
// Two-step: (1) reverse-geocode coords to a tract, then (2) pull ACS 5-yr
// detailed tables for that tract. Per-city tract id is saved on each metric's
// envelope as a meta field so provenance is auditable.
//
// Five taxonomy metrics under Realness + Fabric:
//   Realness: core_density, owner_occ_pct, seasonal_vac_pct, median_price_usd
//   Fabric:   pre1940_pct
// Plus three bonus metrics that don't appear in the headline taxonomy but feed
// chips / detail page: median_income_usd, walk_transit_commute_pct,
// price_to_income_ratio.
//
// Also fills the city-wide population columns (population_total /
// population_source) via the 3-tier Place→cousub→ZCTA cascade. This is the
// per-capita denominator the crowd_season / crowd_intensity pipeline needs;
// it lives here so onboarding a city through scripts/onboard.mjs fills it,
// rather than depending on the Python crowd-season sidecar.

import { measureCensus, measurePlacePopulation } from "../measure.js";

const TAXONOMY_KEYS = [
  "core_density", "owner_occ_pct", "seasonal_vac_pct", "median_price_usd", "pre1940_pct",
];
const BONUS_KEYS = [
  "median_income_usd", "walk_transit_commute_pct", "price_to_income_ratio",
];

export default {
  id: "census",
  describe: "US Census ACS tract metrics + city-wide population (Place→cousub→ZCTA)",
  needs: ["lat", "lon"],
  writes: {
    measuredMetrics: [...TAXONOMY_KEYS, ...BONUS_KEYS],
    columns: ["population_total", "population_source"],
  },
  // Census Geocoder + ACS API are both generous (10k+/day). 800ms is a polite
  // floor.
  throttleMs: 800,
  async run({ lat, lon, asOf, env }) {
    const apiKey = env?.CENSUS_API_KEY;
    if (!apiKey) return { notes: "CENSUS_API_KEY not set — skipped" };
    const { metrics, tract } = await measureCensus(lat, lon, apiKey, { asOf });
    const { population, source } = await measurePlacePopulation(lat, lon, apiKey);

    const patch = {};
    if (Object.keys(metrics).length) patch.measuredMetrics = metrics; // already envelopes
    if (population != null) patch.columns = { population_total: population, population_source: source };
    if (!patch.measuredMetrics && !patch.columns) {
      return { notes: tract ? `tract ${tract} returned no metrics; no population` : "no tract/place for these coords" };
    }
    patch.notes = `tract ${tract ?? "?"} | dens ${metrics.core_density?.value ?? "?"}/sqmi | pop ${population ?? "?"}`;
    return patch;
  },
};

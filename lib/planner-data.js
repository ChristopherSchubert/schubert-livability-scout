...
//
// Upper-end penalty uses the "felt" high: max(hi, heatIndex). A Charleston
// July afternoon at 91°F air temp but 100°F heat index should score the way
// 100°F scores, not the way 91°F does. Dry-air cities are unaffected
// (heatIndex ≈ hi). Below ideal, we still use the dry hi — cold doesn't
// "feel colder" by this mechanism (windchill is a separate signal).
export function monthComfort(m) {
  if (!m || m.hi == null) return null;
  let s = 5;
  const feltHi = m.heatIndex != null && m.heatIndex > m.hi ? m.heatIndex : m.hi;
  const upper = Math.max(0, feltHi - 74);
  const lower = Math.max(0, 74 - m.hi);
  s -= (upper + lower) / 7;

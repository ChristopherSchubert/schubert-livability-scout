...
export const metricTaxonomy = [
  {
    ...
    metrics: [
      ...
      { key: "water_horizon_pct", label: "Water on the horizon",    unit: "%",   dir: +1, source: "Open-Meteo elevation + OSM water", sourceUrl: "https://overpass-api.de" },
    ],
  },
  {
...
export const metricMethod = {
  ...
  water_horizon_pct:"Share of the horizon where you look out onto open water — the sea, a sizable lake, or a real river running through the town (its channel within ~2.5 km). A coastal town's seaward arc, or a riverfront town's water side; 0 for landlocked places.",
...
export const metricScoreBands = {
  ...
  water_horizon_pct:[0, 35],         // none → 0; open water across ~35%+ of the view → 10
...
// "Visit now" — how good THIS month is to visit, purely from climate comfort
// (0–10). Informational: lets you sort for "where's prime right now," and is
// deliberately NOT folded into the measured fit. Null if no climate data.
export function visitNowScore(cityItem, monthIndex) {
  const vc = cityItem.visitClimate;
  if (!Array.isArray(vc) || vc.length !== 12 || !vc[monthIndex]) return null;
  const comfort = monthComfort(vc[monthIndex]); // 0–5
  return comfort == null ? null : Math.round(comfort * 2 * 10) / 10; // → 0–10
}

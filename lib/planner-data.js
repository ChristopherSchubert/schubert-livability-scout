...
export const metricTaxonomy = [
  {
    ...
    metrics: [
      { key: "relief_std_m",   label: "Terrain relief (std dev)",  unit: "m",   dir: +1, source: "USGS 3DEP / SRTM DEM",        sourceUrl: "https://www.usgs.gov/3d-elevation-program" },
      { key: "relief_range_m", label: "Elevation range",            unit: "m",   dir: +1, source: "USGS 3DEP / SRTM DEM",        sourceUrl: "https://www.usgs.gov/3d-elevation-program" },
      ...
      { key: "viewshed_km2",   label: "Viewshed area",              unit: "km²", dir: +1, source: "DEM viewshed (GRASS r.viewshed)", sourceUrl: "https://grass.osgeo.org" },
    ],
  },
  {
...
export const metricMethod = {
  ...
  water_dist_m:    "Straight-line distance to the nearest shoreline point of any lake, river, or coast within 8 km (measured to the actual edge, not the centroid).",
  viewshed_km2:    "Visible land/water area from the core via a digital-elevation line-of-sight model. Not yet computed.",
...
// Per-metric distribution across a set of cities: min / max / sorted values.
// Used to position each city's value within the candidate field.
export function metricFieldStats(cities) {
  const stats = {};
  for (const group of metricTaxonomy) {
    for (const m of group.metrics) {
      const values = cities.map((c) => c.measuredMetrics?.[m.key]?.value).filter((v) => v != null);
      stats[m.key] = values.length ? { min: Math.min(...values), max: Math.max(...values), values } : null;
    }
  }
  return stats;
}

// Relative goodness 0..1 of a value within the field, honoring direction.
export function relGoodness(value, metric, st) {
  if (value == null || !st || st.max === st.min) return null;
  const t = (value - st.min) / (st.max - st.min);
  return metric.dir >= 0 ? t : 1 - t;
}

// Per-axis rollup 0–10: average relative-goodness of that axis's measured
// metrics. Relative to the current candidate field (updates as cities change).
export function axisRollup(cityItem, stats) {
  const out = {};
  for (const group of metricTaxonomy) {
    const scores = [];
    for (const m of group.metrics) {
      const v = cityItem.measuredMetrics?.[m.key]?.value;
      const g = relGoodness(v, m, stats[m.key]);
      if (g != null) scores.push(g);
    }
    out[group.axis] = scores.length ? Math.round((scores.reduce((s, x) => s + x, 0) / scores.length) * 100) / 10 : null;
  }
  return out;
}

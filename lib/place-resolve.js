...
// Rich mask: adds googleMapsUri (higher-SKU). Only requested when opts.rich is
// true so the existing EntryEditor searches stay at the default cost tier.
const FIELD_MASK_RICH = FIELD_MASK + ",places.googleMapsUri";
...
// resolved candidate can be upserted straight into `pois`). When the rich
// mask was used, googleMapsUri will be present on the raw place object.
function toCandidate(p) {
  ...
  return {
    ...
    // Only present when opts.rich was passed to searchPlaces:
...
export async function searchPlaces(textQuery, opts = {}) {
  ...
  if (!q) return [];
  const { near, limit = 8, rich = false } = opts;
  const body = {
  ...
  const r = await fetch(TEXT_SEARCH, {
    method: "POST",
    headers: { "Content-Type": "application/json", "X-Goog-Api-Key": apiKey(opts), "X-Goog-FieldMask": rich ? FIELD_MASK_RICH : FIELD_MASK },

...
const TRIP_COL = {
  ...
  regions: "regions", travelers: "travelers", passes: "passes",
};
...
// ── Fork comments (shared, per-fork discussion thread) ───────────────────────
import { rowToForkComment, forkCommentToRow } from "./fork-comments.js";

// All comments for a trip, newest-last within each fork (sorted by created_at asc).
export async function fetchForkComments(tripId) {
  const sb = getSupabase();
  const { data, error } = await sb
    .from("trip_fork_comments")
    .select("*")
    .eq("trip_id", tripId)
    .order("created_at", { ascending: true });
  if (error) throw error;
  return (data || []).map(rowToForkComment);
}

// Insert a new comment; returns the saved row (with its server id + timestamps).
export async function addForkComment({ tripId, forkId, choiceId, body, lean, authorId }) {
  const sb = getSupabase();
  const { data, error } = await sb
    .from("trip_fork_comments")
    .insert(forkCommentToRow({ tripId, forkId, choiceId, body, lean, authorId }))
    .select("*")
    .single();
  if (error) throw error;
  return rowToForkComment(data);
}

// Remove a comment by id (only the owner can remove, enforced by RLS).
export async function removeForkComment(id) {
  const sb = getSupabase();
  const { error } = await sb.from("trip_fork_comments").delete().eq("id", id);
  if (error) throw error;
}
...
export function subscribeTrip(id, onChange) {
  ...
  const channel = sb.channel(`trip:${id}`)
    .on("postgres_changes",
        { event: "*", schema: "travel", table: "trip_entries", filter: `trip_id=eq.${id}` },
        (p) => onChange({
    ...
    .on("postgres_changes",
        { event: "UPDATE", schema: "travel", table: "trips", filter: `id=eq.${id}` },

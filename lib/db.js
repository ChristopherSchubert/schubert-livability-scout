...
import { rowToTrip, tripToRow, rowToEntry, entryToRow } from "./trip.js";
...
function mapPatch(patch) {
  const M = {
    ...
    blocks: "blocks", blockGeometries: "block_geometries", blockBlurbs: "block_blurbs", poiPositions: "poi_positions", status: "status", decision: "decision", isCalibration: "is_calibration", heroImage: "hero_image",
...
// Partial trip-FRAME patch. mapTripPatch keeps only known frame columns
// (unknown keys dropped, not thrown — the mapPatch discipline); `entries` is
// intentionally not mappable here (entries live in trip_entries).
export async function updateTrip(id, patch) {
  const sb = getSupabase();
  const mapped = mapTripPatch(patch);

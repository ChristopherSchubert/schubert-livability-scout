...
function mapPatch(patch) {
  const M = {
    ...
    tripWeek: "trip_week", why: "why", ifWins: "if_wins", ifFails: "if_fails",
    blocks: "blocks", status: "status", decision: "decision", isCalibration: "is_calibration", heroImage: "hero_image",
    ...
    logisticsNotes: "logistics_notes", days: "days", checklists: "checklists", matrix: "matrix",
    measured: "measured", measuredMetrics: "measured_metrics", waterTarget: "water_target",
    stayZoneBoundary: "stay_zone_boundary",
    horizonFeatures: "horizon_features", lat: "lat", lon: "lon",
    visitClimate: "visit_climate", crowdSeason: "crowd_season", seasonNotes: "season_notes",
  };
  const out = {};
  for (const [k, v] of Object.entries(patch)) {
    if (M[k]) out[M[k]] = v === undefined ? null : v;
  }
  return out;
}
